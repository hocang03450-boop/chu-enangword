...
import { extractContentWithSmartCrop } from './services/geminiService';
    ...
    <div className="min-h-screen flex flex-col transition-colors duration-300">
      ...
      <main className="flex-grow p-4 md:p-8 max-w-7xl mx-auto w-full">
        <div className="mb-8 flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-700">Chuyển đổi Tài liệu sang Word</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="flex flex-col gap-6">
                ...
                {isPreparingPreview ? (
                    <div className="h-[300px] flex flex-col items-center justify-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                        <div className={`w-10 h-10 border-4 border-${theme}-200 border-t-${theme}-600 rounded-full animate-spin mb-3`}></div>
                        <p className="text-sm">{progressMsg}</p>
                    </div>
                ) : (
                    <FileUpload onFileSelect={handleFileSelect} selectedFile={fileData} disabled={status === Status.PROCESSING} />
                )}
                
                {fileData && status !== Status.SUCCESS && !isPreparingPreview && (
                    <div className="flex gap-2">
                        <Button onClick={handleConvert} isLoading={status === Status.PROCESSING} className="flex-grow text-lg shadow-lg">Bắt đầu xử lý</Button>
                        <Button variant="secondary" onClick={() => { setCropIntent('process'); setIsCropping(true); }} disabled={status === Status.PROCESSING}>
            ...
            <div className="flex flex-col gap-6">
                <div className="flex items-center justify-between">
                    ...
                    {fileData && (
                        <Button variant="outline" className="text-xs" onClick={() => { setCropIntent('insert'); setIsCropping(true); }}>
                            <PlusCircle className="w-4 h-4 mr-1" />Cắt & Chèn ảnh
                        </Button>
                    )}
                </div>
                {result ? (
                    ...
                    <div className="flex-grow min-h-[400px] bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center text-gray-400 p-8 text-center">
                        {status === Status.PROCESSING ? (
                            <div className="flex flex-col items-center gap-3 animate-pulse">
                                <div className={`w-12 h-12 border-4 border-${theme}-200 border-t-${theme}-600 rounded-full animate-spin`}></div>
                                <p className={`text-${theme}-600 font-medium`}>{progressMsg}</p>
                            </div>
                        ) : (
                            <>
                                <ArrowRight className="w-12 h-12 mb-4 opacity-20" />
                                <p>Kết quả Word sẽ xuất hiện ở đây sau khi bạn nhấn "Bắt đầu xử lý"</p>
                            </>
                        )}
                    </div>
                )}
      ...
      <footer className="py-4 text-center text-xs text-gray-400 border-t border-gray-100 mt-auto">
        © 2025 Thầy Hồ Cang - THPT Chu Văn An | Powered by Gemini 2.5 Flash
      </footer>
    </div>
  );
};
